...
Gmap
retreive_cached_hits_booleannot ( Search s, N8 bn, Word w, N32 n, Gmap map, Gmap res, N32 *howmany )
{
  Gmap hits    = NULL; 
  Z32 *map_ptr = gm_get_cur_pos ( map );
  Z32 *res_ptr = NULL;
  ...
  while (!s->hit_def->levels[bn].h2m_cmp_func (map_ptr, gm_get_pos ( w->dir, n ), s->hit_def, bn))
    {
      ...
      // bad...doesn't check for bounds of map.
      //      map_ptr += map->gm_f;
    }
...
Gmap
retreive_hit_block ( Search s, N8 bn, Word w, N32 n, Gmap map, N32 *howmany )
{
  Z32 status;
  Z32 *map_ptr;
  Z32 map_pos;
...
Gmap
retreive_cached_hits ( Search s, N8 bn, Word w, N32 n, Gmap map, N32 *howmany )
{
  Gmap hits = NULL;
  Z32 *map_ptr;
  Z32 map_pos;
	  ...
	  if ( !gm_inc_pos ( map ) ) 
	    {
	      ...
	      /*
	      s_log ( s->debug, L_INFO, "cached index 0=%d", (Z8 *)w->cached[0].index[0]);
	      s_log ( s->debug, L_INFO, "cached index 1=%d", (Z8 *)w->cached[0].index[1]);
	      s_log ( s->debug, L_INFO, "cached index 2=%d", (Z8 *)w->cached[0].index[2]);
	      */
